...
return [
  ...
  'fieldMethods' => [
    'handleLinks' => function(Field $field): string {
      ...
      $getPluginOptions = function(): array {
        return [
      ...
      $logError = function(string $message) use ($getPluginOptions): void {
      ...
      $validateServiceConfig = function(array $config, string $service) use ($logError): bool {
      ...
      $sanitizeHandleInput = function(string $input): string {
      ...
      $isValidHandle = function(string $user, string $instance): bool {
      ...
      $createGenericFediverseLink = function(string $user, string $instance) use ($sanitizeHandleInput): string {
      ...
      $createHandleLink = function(string $user, string $instance) use ($logError, $isValidHandle, $getPluginOptions, $validateServiceConfig, $sanitizeHandleInput, $createGenericFediverseLink): string {
        // Validate input
      ...
      $protectCodeBlocks = function(string $text, array &$protected): string {
      ...
      $restoreProtectedContent = function(string $text, array $protected): string {
      ...
      $transformHandles = function(string $text) use ($getPluginOptions, $validateServiceConfig, $isValidHandle, $sanitizeHandleInput, $createGenericFediverseLink): string {
        $options = $getPluginOptions();
        $services = $options['services'];
        ...
        foreach ($services as $domain => $config) {
          ...
          $text = preg_replace_callback($pattern, function($matches) use ($config, $domain, $isValidHandle, $sanitizeHandleInput) {
            $user = $matches[1];
        ...
        if ($options['enableFediverse']) {
          $text = preg_replace_callback(
            '/@([\p{L}\p{N}_]+)@([\p{L}\p{N}.\-]+)/u',
            function($matches) use ($isValidHandle, $createGenericFediverseLink) {
              $user = $matches[1];
              $instance = $matches[2];
      ...
      $processHandleText = function(string $text) use ($getPluginOptions, $logError, $protectCodeBlocks, $transformHandles, $restoreProtectedContent): string {
        $options = $getPluginOptions();
  ...
  'hooks' => [
    'kirbytext:before' => function(string|null $text): string {
      ...
      // Reuse the same logic from fieldMethods
      $field = new Field(null, '', $text);
  ...
  'tags' => [
    'handle' => [
      ...
      'html' => function($tag): string {
        ...
        // Reuse the same logic from fieldMethods
        $field = new Field(null, '', "@{$user}@{$instance}");
        return $field->handleLinks();
      }
    ]
  ]
